            ...
            <div class="col-md-8">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover table-striped caption-top">
                        ...
                        {{-- Table body --}}
                        <tbody>
                            @foreach ($knowledged as $knowledge)
                                <tr>
                                    <td>{{ $knowledge->code }}</td>
                                    <td>{{ $knowledge->short_name }}</td>
                                    <td>{{ $knowledge->statement }}</td>
                                    <td>
                                        <ul class="list-inline m-0">
                                            <li class="list-inline-item">
                                                <a href="/knowledged/{{ $knowledge->id }}"
                                                    class="btn btn-primary btn-sm rounded-pill">
                                                    <i class="fa fa-eye"></i> View
                                                </a>
                                            </li>
                                            <li class="list-inline-item">
                                                <a href="/knowledged/{{ $knowledge->id }}/edit"
                                            ...
                                            <li class="list-inline-item">
                                                ...
                                                <form id="delete-form-{{ $knowledge->id }}"
                                                    action="/knowledged/{{ $knowledge->id }}" method="POST"
                                                    class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
